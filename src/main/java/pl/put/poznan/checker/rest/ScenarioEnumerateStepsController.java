...
@RestController
@RequestMapping(value = { "/enumeratedSteps" })

public class ScenarioEnumerateStepsController {
    ...
    @Autowired
    public ScenarioEnumerateStepsController(ScenarioQualityChecker scenarioQualityChecker) {
    ...
    public ResponseEntity get(@PathVariable(value = "id", required = false) String id,
        ...
        try {
            return new ResponseEntity<>(scenarioQualityChecker.enumerateSteps(), HttpStatus.OK);
