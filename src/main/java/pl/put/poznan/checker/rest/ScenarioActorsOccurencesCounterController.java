...
@RestController
@RequestMapping(value = { "/ActorsOccurencesCount" })

public class ScenarioActorsOccurencesCounterController {

    private static final Logger logger = LoggerFactory.getLogger(ScenarioActorsOccurencesCounterController.class);
    private final ScenarioQualityChecker scenarioQualityChecker;

    @Autowired
    public ScenarioActorsOccurencesCounterController(ScenarioQualityChecker scenarioQualityChecker) {
    ...
    public ResponseEntity get(@PathVariable(value = "id", required = false) String id,
        ...
        logger.info("Received a request to /ActorsOccurencesCount");
        try {
            return new ResponseEntity<>(new ActorsOccurencesCounterOutput(scenarioQualityChecker.calculateActorsOccurences()),
                    HttpStatus.OK);
